...
def create_dataset_singleLine(path, num_examples):
    """
    # 1. Remove the accents
    # 2. Clean the sentences
    # 3. Return word pairs in the format: [ENGLISH, SPANISH]
    ...
    lines = open(path, 'r', encoding='utf-8').read().strip().split('\n')
    source_texts=[preprocess_sentence(l)  for l in lines[:num_examples]]
...
def show_progress(curr, total, time=""):
    ...
    sys.stdout.flush()
source_texts,target_texts=create_dataset("train.txt",2) 
print(source_texts  )